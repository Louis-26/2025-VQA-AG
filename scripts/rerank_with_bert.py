...
def rerank_by_bertscore(df: pd.DataFrame, gt_map: Dict[str, str], bert_score, model_type: str = None, rescale_with_baseline: bool = True, use_idf: bool = False) -> pd.DataFrame:
    ...
    for qid, group in tqdm(groups):
        ...
        bert_scores = []
        gt_answer_list = [gt_answer] * len(answers)


        P, R, F1 = bert_score(answers, gt_answer_list, **kwargs)




        temp_df = group.copy()
        temp_df["BERTScore_F1"] = F1.tolist()
        

        temp_df = temp_df.sort_values("BERTScore_F1", ascending=False).reset_index(drop=True)
...
def main():
    ...
    if args.enable_rerank:
        df = rerank_by_bertscore(df, gt_map, mods["bert_score"],
