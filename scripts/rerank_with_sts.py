...
def rerank_by_sts(df: pd.DataFrame, gt_map: Dict[str, str], SentenceTransformer, np, model_name: str) -> pd.DataFrame:
    ...
    for qid, group in groups:
        ...
        gt_answer = gt_map.get(str(qid).strip())
            

        answers = group["Answer"].fillna("None").tolist()
        sts_scores = []

        for answer in answers:

            emb_answer = model.encode([answer], convert_to_numpy=True, normalize_embeddings=True)
            emb_gt = model.encode([gt_answer], convert_to_numpy=True, normalize_embeddings=True)
            

            sts_score = float((emb_answer * emb_gt).sum(axis=1)[0])
        ...
        temp_df = group.copy()
        temp_df["STS_Score"] = sts_scores
        

        temp_df = temp_df.sort_values("STS_Score", ascending=False).reset_index(drop=True)
        

        temp_df["New_Rank"] = range(1, len(temp_df) + 1)
        
        reranked_rows.append(temp_df)
...
def main():
    ...
    parser.add_argument("--reranked_out", default=None, help="Optional path to write reranked CSV")
    parser.add_argument("--enable_rerank", action="store_true", help="Enable STS-based reranking")
    ...
    parser.add_argument("--normalize", action="store_true", help="Also compute normalized text metrics")
    ...
    if args.enable_rerank:
        df = rerank_by_sts(df, gt_map, mods["SentenceTransformer"], mods["np"], args.sts_model)
        
        if args.reranked_out is None:
            base = os.path.splitext(args.pred_file)[0]
            args.reranked_out = base + "_reranked.csv"
        df.to_csv(args.reranked_out, index=False)
    ...
    if args.enable_rerank:
        print(f"Reranking enabled: Results reranked by STS scores")
    print("Averages (raw):")
    ...
    if args.enable_rerank:
        print(f"Reranked CSV: {args.reranked_out}")
