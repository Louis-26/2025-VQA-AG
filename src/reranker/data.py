...
class VideoPointerCollator:
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        texts: List[str] = []
        videos: List[Any] = []  # numpy arrays (T, H, W, C)
        targets: List[str] = []
        ...
        # If all samples in this batch failed decoding, create a single dummy sample
        if len(videos) == 0:
            import numpy as np
        ...
        messages = []
        for text, video in zip(texts, videos):
            messages.append({
                "role": "user",
                "content": [
        ...
        model_inputs = self.processor(text=input_texts, videos=videos, return_tensors="pt", padding=True)
